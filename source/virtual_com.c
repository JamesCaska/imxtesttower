...
const uint8_t  SLAVE_COUNT = 10;
uint8_t slaveSelectPins[] = {8, 1, 15, 16, 128 + 2, 3, 21, 23, 24, 28};

/*
 * PROTOCOL DECODER
 */
uint8_t  _frameBuffer[1000]; //Max frame = 10 * 65 + CRC + ENCRYPTION_PADDING
uint16_t _frameWritePos;

uint8_t  _outgoingBuffer[1000]; //Max frame = 10 * 65 + CRC + ENCRYPTION_PADDING
uint16_t _outgoingWritePos;
uint16_t _outgoingReadPos;

uint16_t usbInReadPos;
...
enum decodeState{ SyncStartFrame, InFrame, Escaped};

enum decodeState inDecoder = SyncStartFrame;
enum decodeState outDecoder = SyncStartFrame;

const uint8_t FRAME_START = 193;
const uint8_t FRAME_END = 192;
const uint8_t FRAME_ESC = 219;

const uint8_t ESC_END = 220;
const uint8_t ESC_ESC = 221;
const uint8_t ESC_START = 222;

/*
 * The incoming stream is broken into frames.
 * The virtual uart is not packetised so we need to be able to detect frames in the stream
 * SLIP frame encoding is used
 *
 * Each Frame has the following format
 * [FRAME_START]
 * ESCAPED( FRAME_ENCRYPTED_DATA )
 * [FRAME_END]
 *
 * FRAME_ENCRYPTED_DATA = ENCRYPED( FRAME_DATA )
 *
 * FRAME_DATA={
 * [SLAVE1 LEN]
 * [SLAVE1 DATA]
...
uint8_t DecodeIncomingFrame(){
	uint8_t nxt;

	 //Append the received Serial to the encodedBuffer

	while(usbInReadPos != s_recvSize )
	{
		nxt = s_currRecvBuf[usbInReadPos++];

		if( inDecoder == SyncStartFrame){
			if( nxt == FRAME_START) inDecoder = InFrame;
		}else{
			if( inDecoder == Escaped){
				if(nxt == ESC_START){
					nxt = FRAME_START;
				}else if( nxt == ESC_END){
					nxt = FRAME_END;
				}else if( nxt == FRAME_ESC){
					nxt = FRAME_ESC;
				}else{
					//Error!
				}
				inDecoder = InFrame;
				_frameBuffer[_frameWritePos++] = nxt;
			}else{
				if(nxt == FRAME_END){
					inDecoder =  SyncStartFrame;
					return _frameWritePos;
				}else if( nxt == ESC_START){
					inDecoder =  Escaped;
				}else{
					_frameBuffer[_frameWritePos++] = nxt;
				}
...
/*
 * Writes a slave response to the outgoing frame slip encoded.
 *
 * Data
 *
 * [Len][0..64 bytes]
 *
 */
void EncodeToOutgoingFrame(){
	uint8_t readPos = 0;
	uint8_t i,len,v;

	len = masterRxData[readPos++];

	//Check if the length is valid..
	if( len < 64){

		_outgoingBuffer[_outgoingWritePos++] = len; //max len = 64 so cannot require escape.

		for(i = 0; i < len; i++ ){
			v = masterRxData[readPos++];
			if(v == FRAME_END){
				_outgoingBuffer[_outgoingWritePos++] = FRAME_ESC;
				_outgoingBuffer[_outgoingWritePos++] = ESC_END;
			}
			else if( v==FRAME_START )
			{
				_outgoingBuffer[_outgoingWritePos++] = FRAME_ESC;
				_outgoingBuffer[_outgoingWritePos++] = ESC_START;
			}
			else if( v==FRAME_ESC )
			{
				_outgoingBuffer[_outgoingWritePos++] = FRAME_ESC;
				_outgoingBuffer[_outgoingWritePos++] = ESC_ESC;
			}
			else
			{
				_outgoingBuffer[_outgoingWritePos++] = v;
			}
		}
	}
...
/*
 * Optionally Decrypt the frame in place
*/
void DecryptFrame(){

}
/*
 * Optionally Encrypt the frame in place
*/
void EncryptFrame(){
...
/*
 * The lengths should all add up to the length of the packet.
 */
uint8_t VerifyFrameFormat(){
	uint16_t pos = 0;
	uint8_t len = 0;
	uint8_t i = 0;
	...
	{
		len = _frameBuffer[pos];
		pos+=len+1;
		if( pos > _frameWritePos) return 0;
	}

	return ( pos == _frameWritePos ) ? 1: 0;

}

/*
 * Select each slave and exchange the data.
 *
 * A full packet is send even if it is not full to allow the slave to return a full packet response.
 *
 */
void ExchangeDataWithSlaves(){
	...
	pos = 0;
	_outgoingBuffer[_outgoingWritePos++] = FRAME_START;

	for(slave = 0; slave < SLAVE_COUNT; slave++ )
	{
		len = _frameBuffer[pos] + 1;

		//Copy the outgoing data chunk for the slave
		//[LEN][DATA]
		for (i = 0; i < len; i++)
		{
			masterTxData[i] = _frameBuffer[pos++];
		}

		pin = slaveSelectPins[slave];

		//Select the Slave
	 	GPIO_PinWrite( ((pin & 0x80) == 0) ? GPIO1 : GPIO2 , pin & 0x7F, 1U);
	   ...
	   // masterRxData[0] = 63;//Simulate full data response.

	    anyData = anyData || (masterRxData[0]!=0);

		//Release the Slave
	 	GPIO_PinWrite( ((pin & 0x80) == 0) ? GPIO1 : GPIO2 , pin & 0x7F, 0U);

		EncodeToOutgoingFrame();

	}

	_outgoingBuffer[_outgoingWritePos++] = FRAME_END;

	//Remove header if no data to send
	if( !anyData) _outgoingWritePos = 0;


}

/**
 * Copies a chunk of the output stream into the Usb output buffer.
 */
uint32_t WriteOutputFrameToUsb(){

	EncryptFrame();

	uint32_t toSend = _outgoingWritePos;

	if( toSend > DATA_BUFF_SIZE ){

		toSend =DATA_BUFF_SIZE;

		memcpy( s_currSendBuf, _outgoingBuffer , DATA_BUFF_SIZE);

		//Shift the overflow to the read head
		memcpy(_outgoingBuffer,  _outgoingBuffer + DATA_BUFF_SIZE , _outgoingWritePos - DATA_BUFF_SIZE);

		_outgoingWritePos-=DATA_BUFF_SIZE;

	}else if( toSend!= 0 ){
		memcpy( s_currSendBuf, _outgoingBuffer + _outgoingReadPos , toSend);
		_outgoingWritePos =0;
	}
...
void APP_task(void)
{
    ...
    if ((1 == s_cdcVcom.attach) && (1 == s_cdcVcom.startTransactions))
    {
        ...
        if ((0 != s_recvSize) && (0xFFFFFFFFU != s_recvSize))
        {
            ...
            s_recvSize = 0;
            //Copy to outgoing buffer.. the worst case 'can be' longer than the max frame..
            s_sendSize = WriteOutputFrameToUsb();

        }
        else if( _frameWritePos==0)
         {
        	//Create dummy frame
        	CreateDummyFrame();
        	ExchangeDataWithSlaves();
